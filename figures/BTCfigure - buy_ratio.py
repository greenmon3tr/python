"""
The code is used for plotting a figure of the returns of the strategy, with different buy-in/buy-out ratio.
"""
import os
import pandas as pd
import numpy as np
    ...
    if df['ret_std'].iloc[i] > 1e-6:
        ...
        if abs(stat1) > threshold and abs(stat1) < bound:
            ...
            if stat1 > 0:  # 如果预测为正且没有持仓
                buy = min(cash1, equity1 * buy_ratio1)
                position1 += ( buy / (price) / (1+fee) ) # 用现金买入比特币
                cash1 -= buy
                buy = min(cash2, equity2 * buy_ratio2)
                position2 += ( buy / (price) / (1+fee) ) # 用现金买入比特币
                cash2 -= buy
                buy = min(cash3, equity3 * buy_ratio3)
                position3 += ( buy / (price) / (1+fee) ) # 用现金买入比特币
                cash3 -= buy
                buy = min(cash4, equity4 * buy_ratio4)
                position4 += ( buy / (price) / (1+fee) ) # 用现金买入比特币
                cash4 -= buy  

            else:  # 如果预测为负且有持仓
                sold = min(position1, equity1 * buy_ratio1 / price)
                cash1 += ( sold * (price) / (1+fee) ) # 卖出比特币
                position1 -= sold
                sold = min(position2, equity2 * buy_ratio2 / price)
                cash2 += ( sold * (price) / (1+fee) ) # 卖出比特币
                position2 -= sold
                sold = min(position3, equity3 * buy_ratio3 / price)
                cash3 += ( sold * (price) / (1+fee) ) # 卖出比特币
                position3 -= sold
                sold = min(position4, equity4 * buy_ratio4 / price)
...
plt.plot(equity_series_1, label=f'buy ratio = {buy_ratio1}', color='red', linewidth=1)
plt.plot(equity_series_2, label=f'buy ratio = {buy_ratio2}', color='orange', linewidth=1)
plt.plot(equity_series_3, label=f'buy ratio = {buy_ratio3}', color='yellow', linewidth=1)
plt.plot(equity_series_4, label=f'buy ratio = {buy_ratio4}', color='green', linewidth=1)
