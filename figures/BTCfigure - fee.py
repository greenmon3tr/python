"""
"The code is used for plotting a figure of the returns of the strategy, with different transction fees.
"""
import os
import pandas as pd
import numpy as np
...
cash1 = initial_cash * leverage
equity2 = initial_cash
cash2 = initial_cash * leverage
equity3 = initial_cash
cash3 = initial_cash * leverage
equity4 = initial_cash
cash4 = initial_cash * leverage  # 初始资金
    ...
    if df['ret_std'].iloc[i] > 1e-6:
        ...
        if abs(stat1) > threshold and abs(stat1) < bound:
            ...
            if stat1 > 0:  # 如果预测为正且没有持仓
                buy = min(cash1, equity1 * buy_ratio * leverage)
                position1 += ( buy / (price) / (1+fee1) ) # 用现金买入比特币
                cash1 -= buy
                buy = min(cash2, equity2 * buy_ratio * leverage)
                position2 += ( buy / (price) / (1+fee2) ) # 用现金买入比特币
                cash2 -= buy
                buy = min(cash3, equity3 * buy_ratio * leverage)
                position3 += ( buy / (price) / (1+fee3) ) # 用现金买入比特币
                cash3 -= buy
                buy = min(cash4, equity4 * buy_ratio * leverage)
                position4 += ( buy / (price) / (1+fee4) ) # 用现金买入比特币
                cash4 -= buy  

            else:  # 如果预测为负且有持仓
                sold = min(position1, equity1 * buy_ratio / price * leverage)
                cash1 += ( sold * (price) / (1+fee1) ) # 卖出比特币
                position1 -= sold
                sold = min(position2, equity2 * buy_ratio / price * leverage)
                cash2 += ( sold * (price) / (1+fee2) ) # 卖出比特币
                position2 -= sold
                sold = min(position3, equity3 * buy_ratio / price * leverage)
                cash3 += ( sold * (price) / (1+fee3) ) # 卖出比特币
                position3 -= sold
                sold = min(position4, equity4 * buy_ratio / price * leverage)
                cash4 += ( sold * (price) / (1+fee4) ) # 卖出比特币
                position4 -= sold

    equity1 = (cash1 + position1 * price - initial_cash * (leverage - 1))
    equity_curves_1.append(equity1)
    equity2 = (cash2 + position2 * price - initial_cash * (leverage - 1))
    equity_curves_2.append(equity2)
    equity3 = (cash3 + position3 * price - initial_cash * (leverage - 1))
    equity_curves_3.append(equity3)
    equity4 = (cash4 + position4 * price - initial_cash * (leverage - 1))
...
plt.plot(equity_series_1, label=f'fee = {fee1}', color='red', linewidth=1)
plt.plot(equity_series_2, label=f'fee = {fee2}', color='orange', linewidth=1)
plt.plot(equity_series_3, label=f'fee = {fee3}', color='yellow', linewidth=1)
plt.plot(equity_series_4, label=f'fee = {fee4}', color='green', linewidth=1)
